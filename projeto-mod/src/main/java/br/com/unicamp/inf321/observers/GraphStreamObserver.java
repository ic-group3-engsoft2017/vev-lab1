...
public class GraphStreamObserver implements Observer {
  ...
  private static String stylesheet =
      "" + "node {" + " shape: rounded-box;" + " fill-color: grey;" + " fill-mode: dyn-plain;"
          + " size-mode: fit;" + "}" + "node.active {" + " fill-color: green;" + " size:40px;" + "}"
          + "edge {" + " shape: cubic-curve;" + " size: 1px;" + " arrow-shape: arrow;" + "}"
          + "edge.active {" + " fill-color: green;" + " size: 5px;" + "}";
  ...
  public void update(Machine machine, Element element, EventType eventType) {
    if (EventType.BEFORE_ELEMENT.equals(eventType)) {
      ...
      if (element instanceof RuntimeVertex) {
        ...
        graph.getNode(getId(machine.getCurrentContext(), vertex)).addAttribute("ui.class",
            "active");

        // Check whether we should draw an edge between 2 shared vertices
        if (vertex.hasSharedState() && lastElement instanceof RuntimeVertex
            && ((RuntimeVertex) lastElement).hasSharedState()
            && machine.getCurrentContext() != lastContext) {
          if (null == graph.getEdge(
        ...
        if (null == graph.getEdge(getId(machine.getCurrentContext(), edge))) {
          ...
          Edge arc = graph.addEdge(getId(machine.getCurrentContext(), edge), source,
              getId(machine.getCurrentContext(), edge.getTargetVertex()), true);
          arc.addAttribute("ui.label", counter++ + "-" + edge.getName());
        }
