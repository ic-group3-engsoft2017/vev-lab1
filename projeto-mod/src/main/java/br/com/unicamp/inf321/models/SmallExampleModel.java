 ...
 *
 * Implements the GraphWalker model: src/main/resources/SmallExample.graphml The
 * SmallExample.graphml can be opened and edited using http://www.yworks.com/en/products/yfiles/yed/
 *
 * For convienicene, a jpg image exists side-by-side in the same folder.
 *
 * The @GraphWalker annotation, has the following meaning: 1) value defines the generator of this
 * test. Please read more on the subject at:
 * http://graphwalker.org/docs/path_generators_and_stop_conditions 2) start defines the first
 * element in the model to be executed. (Element is either a vertex or an edge)
 *
 * The interface SmallTest, that SomeSmallTest implements, is generated by running: mvn
 * graphwalker:generate-sources also: mvn test
 *
 */
