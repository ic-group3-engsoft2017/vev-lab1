 ...
 * @author Nils Olsson
 */
public class WebSocketObserver extends org.java_websocket.server.WebSocketServer
    implements
      Observer {
  ...
  public void onOpen(WebSocket socket, ClientHandshake handshake) {
    sockets.add(socket);
    logger
        .info(socket.getRemoteSocketAddress().getAddress().getHostAddress() + " is now connected");
  }
  ...
  public void onClose(WebSocket socket, int code, String reason, boolean remote) {
    ...
    logger
        .info(socket.getRemoteSocketAddress().getAddress().getHostAddress() + " has disconnected");
  }

  @Override
  public void onMessage(WebSocket socket, String message) {
    logger.debug("Received message from: "
        + socket.getRemoteSocketAddress().getAddress().getHostAddress() + " : " + message);
  }
  ...
  public void update(Machine machine, Element element, EventType eventType) {
    if (EventType.BEFORE_ELEMENT.equals(eventType)) {
      ...
      if (element instanceof Vertex.RuntimeVertex) {
        ...
        if (edge.getSourceVertex() != null && edge.getTargetVertex() != null) {
          jsonObject.put("type", "edge");
